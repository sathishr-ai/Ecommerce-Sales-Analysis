(function () {
            ...
            let catChartInst, countryChartInst;
            ...
            function aggregateData(rows) {
                ...
                for (let c in countrySales) {
                    if (countrySales[c] > maxCountry) { maxCountry = countrySales[c]; topCountry = c; }
                }
                ...
                const countryEntries = Object.entries(countrySales).sort((a, b) => b[1] - a[1]).slice(0, 7);
                const countryNames = countryEntries.map(e => e[0]);
                const countryValues = countryEntries.map(e => (e[1] / totalSales * 100).toFixed(1));
        ...
        `;
                ...
                catChartInst = new Chart(document.getElementById('catChart'), {
                    type: 'bar',
                    data: { labels: data.catNames, datasets: [{ label: 'Sales (thousand $)', data: data.catValues, backgroundColor: '#1D4E6F', borderRadius: 8 }] },
                ...
                countryChartInst = new Chart(document.getElementById('countryChart'), {
                    type: 'doughnut',
                    data: { labels: data.countryNames, datasets: [{ data: data.countryValues, backgroundColor: ['#0A2647', '#1b4d6e', '#2d6f93', '#4287b0', '#5fa0c5', '#7fb6d6', '#a1cce7'], borderWidth: 0 }] },
